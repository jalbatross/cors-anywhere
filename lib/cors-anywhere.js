...
function withCORS(headers, request) {
  ...
  if (request.headers["set-cookie"] && !headers["set-cookie"]) {
    headers["set-cookie"] = request.headers["set-cookie"];
  }
  if (headers["set-cookie"]) {
    let big = "";
    request.headers["set-cookie"] = headers["set-cookie"];
    headers["set-cookie"].forEach((val, i) => {
      headers[`fake-cookie-${i}`] = val;
      request.headers[`fake-bagel-${i}`] = val;
      big += val.toString();
    });

    if (big) {
      headers["big-header"] = big;
      request.headers["bigger-header"] = big;
    }
    console.log("Returning these headers: ", headers);
    console.log("header for set cookie: ", headers["set-cookie"]);
    headers["my-favorite-header"] = "A nookie;";
    headers["my-other-favorite-header"] = "A cookie;";
  }
...
function proxyRequest(req, res, proxy) {
  ...
  try {
    console.log("proxy web ccalled: ", proxy);
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Strip cookies
  console.log("Did not strip cookies");
  console.log("cookie1: ", proxyRes.headers["set-cookie"]);
  console.log("cookie2: ", proxyRes.headers["set-cookie2"]);
  let body = "";
  if (proxyRes.headers["set-cookie"]) {
    proxyRes.headers["set-cookie"].forEach((val, i) => {
      res.setHeader(`fake-cookie-${i}`, val);
      body += val.toString();
    });
  }

  res.setHeader("set-cookie", proxyRes.headers["set-cookie"]);
  res.setHeader("myheader", body);
  res.setHeader("other-header", "please work");
  proxyRes.headers["x-final-url"] = "changed";
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    console.log("Cors headers now: ", cors_headers);
    ...
    console.log("Proxying the request now");
    proxyRequest(req, res, proxy);
  };
}
